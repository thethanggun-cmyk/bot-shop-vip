...
function renderAccCategories() {
        const box = document.getElementById("accCategoryListContainer");
        if (!box) return;
        const cats = Array.isArray(appData.accCategories) ? appData.accCategories : [];
        box.innerHTML = "";
        const all = document.createElement("button");
        all.className = `category-chip ${selectedAccCategoryId === "all" ? "active" : ""}`;
        all.textContent = "📦 Tất cả";
        all.onclick = () => { selectedAccCategoryId = "all"; renderAccCategories(); renderAccList(); };
        box.appendChild(all);
        cats.forEach(c => {
            const b = document.createElement("button");
            b.className = `category-chip ${selectedAccCategoryId === String(c.id) ? "active" : ""}`;
            b.textContent = `${c.icon || "👤"} ${c.name}`;
            b.onclick = () => { selectedAccCategoryId = String(c.id); renderAccCategories(); renderAccList(); };
            box.appendChild(b);
        });
    }
...
function renderAccList() {
    ...
    if (selectedAccCategoryId !== "all") {
        accProds = accProds.filter(p => String(p.categoryId || "") === String(selectedAccCategoryId));
    }
