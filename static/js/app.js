...
// Danh mục game cố định - không cần lệnh Telegram để tạo/xóa.
const FIXED_GAME_CATEGORIES = [
    { id: 'freefire', name: 'Free Fire', icon: '/static/img/game_icons/free-fire.jpg' },
    { id: 'lienquan', name: 'Liên Quân AOV', icon: '/static/img/game_icons/lien-quan.jpg' },
    { id: 'pubg', name: 'Pubg', icon: '/static/img/game_icons/pubg.jpg' },
    { id: 'playtogether', name: 'Play Together', icon: '/static/img/game_icons/play-together.jpg' },
    { id: 'zingspeed', name: 'Zing Speed', icon: '/static/img/game_icons/zing-speed.jpg' }
];

function normalizeGameCategoryId(value) {
    const v = String(value || '').trim().toLowerCase();
    const aliases = {
        'free fire': 'freefire', 'free-fire': 'freefire', 'free_fire': 'freefire',
        'liên quân': 'lienquan', 'lien quan': 'lienquan', 'lien-quan': 'lienquan', 'lienquan aov': 'lienquan',
        'pubg mobile': 'pubg', 'pubg': 'pubg',
        'play together': 'playtogether', 'play-together': 'playtogether', 'play_together': 'playtogether',
        'zing speed': 'zingspeed', 'zing-speed': 'zingspeed', 'zing_speed': 'zingspeed'
    };
    return aliases[v] || v;
}
...
function renderCategories() {
    ...
    const keyProducts = (appData.products || []).filter(p => p.type !== 'acc');
    if (countBadge) countBadge.innerText = FIXED_GAME_CATEGORIES.length + ' mục';
    container.innerHTML = '';

    FIXED_GAME_CATEGORIES.forEach((cat) => {
        const count = keyProducts.filter(p => normalizeGameCategoryId(p.categoryId) === cat.id).length;
        const card = document.createElement('button');
        card.type = 'button';
        card.className = 'btn-press cursor-pointer w-full min-w-0 p-3 rounded-2xl bg-[#080c14] border border-[#1a2a44] shadow-sm flex flex-col items-center justify-center gap-2 transition hover:border-cyan-400/50 ' + (selectedCategoryId === cat.id ? 'border-cyan-400 bg-cyan-500/10' : '');
        card.onclick = () => { selectedCategoryId = cat.id; renderCategories(); renderProducts(); };
        card.innerHTML = '<img src="' + cat.icon + '" alt="' + escapeHtml(cat.name) + '" class="w-[58px] h-[58px] rounded-2xl object-cover border border-white/10 shadow-lg" onerror="this.style.display=\'none\'">' +
            '<span class="text-[11px] sm:text-xs font-bold text-white text-center leading-tight">' + escapeHtml(cat.name) + '</span>' +
            '<span class="text-[10px] text-gray-400 font-num">' + count + ' mục</span>';
...
function renderProducts() {
    ...
    const filtered = allProds.filter(p => {
        ...
        return normalizeGameCategoryId(p.categoryId) === selectedCategoryId;
    });
...
function renderAccCategories() {
    // Render the fixed game cards independently from product/API data.
    // This guarantees the game selector is visible even when the API returns
    // no products or another renderer encounters an error.
    const container = document.getElementById('accCategoryListContainer');
    if (!container) return;

    const products = Array.isArray(appData.products) ? appData.products : [];
    const accProducts = products.filter(p => p && p.type === 'acc');
    container.innerHTML = '';

    FIXED_GAME_CATEGORIES.forEach((cat) => {
        const count = accProducts.filter(p => normalizeGameCategoryId(p.categoryId) === cat.id).length;
        const card = document.createElement('button');
        card.type = 'button';
        card.className = 'btn-press cursor-pointer w-full min-w-0 p-3 rounded-2xl bg-[#080c14] border border-[#1a2a44] shadow-sm flex flex-col items-center justify-center gap-2 transition hover:border-cyan-400/50 ' + (selectedAccCategoryId === cat.id ? 'border-cyan-400 bg-cyan-500/10' : '');
        card.addEventListener('click', () => {
            selectedAccCategoryId = cat.id;
            renderAccCategories();
            renderAccList();
        });
        card.innerHTML =
            '<img src="' + cat.icon + '" alt="' + escapeHtml(cat.name) + '" class="w-[58px] h-[58px] rounded-2xl object-cover border border-white/10 shadow-lg" onerror="this.style.display=\'none\'">' +
            '<span class="text-[11px] sm:text-xs font-bold text-white text-center leading-tight">' + escapeHtml(cat.name) + '</span>' +
            '<span class="text-[10px] text-gray-400 font-num">' + count + ' mục</span>';
        container.appendChild(card);
    });
...
function renderAccList() {
    ...
    if (!container) return;
    container.innerHTML = '';
    let accProds = (appData.products || []).filter(p => p.type === 'acc');
    if (selectedAccCategoryId !== 'all') {
        accProds = accProds.filter(p => normalizeGameCategoryId(p.categoryId) === selectedAccCategoryId);
    }
...
function refreshAllUI() {
    ...
    // Render fixed account games first so they are not blocked by product/API rendering.
    try { renderAccCategories(); } catch (e) { console.error('renderAccCategories:', e); }
    try { renderAccList(); } catch (e) { console.error('renderAccList:', e); }
    try { renderCategories(); } catch (e) { console.error('renderCategories:', e); }
