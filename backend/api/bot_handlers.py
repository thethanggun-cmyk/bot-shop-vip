...
async def themdanhmucmuaacc_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not raw or "|" not in raw:
        await update.message.reply_text(
            f"{TG_EMOJI['WARNING']} Cú pháp: <code>/themdanhmucmuaacc Mã_DM | Tên_DM | Icon</code>",
            parse_mode="HTML"
        )
        return
    ...
    cat_id = parts[0].lower()
    cat_name = parts[1]
    cat_icon = parts[2] if len(parts) > 2 and parts[2] else "fa-id-card"

    store_data = load_server_store_data()
    cats = store_data.setdefault("accCategories", [])

    if any(c.get("id") == cat_id for c in cats):
        await update.message.reply_text(
            f"{TG_EMOJI['WARNING']} Mã danh mục Acc <code>{cat_id}</code> đã tồn tại!",
            parse_mode="HTML"
        )
        return
    ...
    await update.message.reply_text(
        f"{TG_EMOJI['CHECK']} Đã thêm danh mục Mua Acc: <b>{cat_name}</b> (<code>{cat_id}</code>)",
        parse_mode="HTML"
    )
