...
async def admin_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    help_msg = (
        ...
        "📁 <b>2. QUẢN LÝ DANH MỤC GAME:</b>\n"
        "• <code>/themdanhmuc &lt;id&gt; | &lt;Tên&gt; | &lt;Icon&gt;</code>\n"
        "• <code>/xoadanhmuc &lt;id&gt;</code> : Xoá danh mục\n"
        "• <code>/dsdanhmuc</code> : Liệt kê toàn bộ danh mục game\n\n"
...
async def themdanhmucmuaacc_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if update.effective_user.id not in ADMIN_IDS:
        return
    raw = (update.effective_message.text or "").split(None, 1)
    if len(raw) < 2 or "|" not in raw[1]:
        await update.effective_message.reply_text(
            f"{TG_EMOJI['WARNING']} <b>CÚ PHÁP:</b> <code>/themdanhmucmuaacc Mã_DM | Tên_DM | Icon</code>",
            parse_mode="HTML"
        )
        return
    parts = [x.strip() for x in raw[1].split("|")]
    if len(parts) < 2 or not parts[0] or not parts[1]:
        await update.effective_message.reply_text(f"{TG_EMOJI['CROSS']} Thiếu Mã_DM hoặc Tên_DM!")
        return
    cat_id = parts[0].lower()
    name = parts[1]
    icon = parts[2] if len(parts) >= 3 and parts[2] else "👤"
    store_data = load_server_store_data()
    categories = store_data.setdefault("accCategories", [])
    if any(str(c.get("id", "")).lower() == cat_id for c in categories):
        await update.effective_message.reply_text(f"{TG_EMOJI['CROSS']} Mã danh mục <code>{cat_id}</code> đã tồn tại!", parse_mode="HTML")
        return
    categories.append({"id": cat_id, "name": name, "icon": icon})
    save_server_store_data(store_data)
    await update.effective_message.reply_text(
        f"{TG_EMOJI['CHECK']} <b>ĐÃ THÊM DANH MỤC MUA ACC!</b>\n"
        f"• Mã: <code>{cat_id}</code>\n• Tên: <b>{name}</b>\n• Icon: {icon}", parse_mode="HTML"
    )
...
async def themdanhmuc_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if update.effective_user.id not in ADMIN_IDS: return

    raw = " ".join(context.args).strip()
    if not raw or "|" not in raw:
        await update.message.reply_text(f"{TG_EMOJI['WARNING']} Cú pháp: <code>/themdanhmuc Mã_DM | Tên_DM | Icon</code>", parse_mode="HTML")
        return

    parts = [p.strip() for p in raw.split("|")]
    cat_id = parts[0].lower()
    cat_name = parts[1]
    cat_icon = parts[2] if len(parts) > 2 else "fa-gamepad"

    store_data = load_server_store_data()
    cats = store_data.setdefault("categories", [])
    cats.append({"id": cat_id, "name": cat_name, "icon": cat_icon})
    save_server_store_data(store_data)

    await update.message.reply_text(f"{TG_EMOJI['CHECK']} Đã thêm danh mục: <b>{cat_name}</b> (<code>{cat_id}</code>)", parse_mode="HTML")

async def xoadanhmuc_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if update.effective_user.id not in ADMIN_IDS: return

    if not context.args:
        await update.message.reply_text(f"{TG_EMOJI['WARNING']} Cú pháp: <code>/xoadanhmuc &lt;Mã_DM&gt;</code>", parse_mode="HTML")
        return

    cat_id = context.args[0].strip().lower()
    store_data = load_server_store_data()
    cats = store_data.get("categories", [])
    store_data["categories"] = [c for c in cats if c.get("id") != cat_id]
    save_server_store_data(store_data)

    await update.message.reply_text(f"🗑️ Đã xoá danh mục <code>{cat_id}</code>!", parse_mode="HTML")

async def dsdanhmuc_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if update.effective_user.id not in ADMIN_IDS: return
    ...
    if not cats:
        await update.message.reply_text(
            "📁 <b>DANH SÁCH DANH MỤC GAME:</b>\nHiện tại chưa có danh mục nào trên hệ thống.",
            parse_mode="HTML"
        )
        return

    msg = f"📁 <b>DANH SÁCH DANH MỤC TRÊN HỆ THỐNG ({len(cats)} mục):</b>\n────────────────────────\n"
    for i, c in enumerate(cats, 1):
        c_id = c.get("id", "")
        c_name = c.get("name", "")
        c_icon = c.get("icon", "fa-gamepad")
        prod_count = len([p for p in prods if p.get("categoryId") == c_id])
        msg += f"{i}. <b>{c_name}</b> (<code>{c_id}</code>)\n"
        msg += f"   • Icon: <code>{c_icon}</code>\n"
        msg += f"   • Số sản phẩm: <b>{prod_count} gói</b>\n\n"
