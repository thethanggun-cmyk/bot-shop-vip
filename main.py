...
db.accounts.create_index([("_id", 1)])
db.accounts_free.create_index([("_id", 1)])
db.deposits.create_index([("userId", 1), ("timestamp", -1)])
...
async def thongbao_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for uid in users:
        try:
            await context.bot.send_message(chat_id=int(uid), text=f"📢 <b>THÔNG BÁO</b>\n\n{content}", parse_mode="HTML")
            ok += 1
        except Exception:
            pass
...
async def user_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"📋 <b>TỔNG SỐ USER:</b> {len(users)}", parse_mode="HTML")

    for idx, chunk in enumerate(chunks, 1):
        msg = f"📋 <b>Nhóm {idx}/{len(chunks)}</b>\n\n"
        for uid in chunk:
            u = get_user(uid)
            try:
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("admin_add_"):
        ...
        await query.edit_message_text(
            f"✏️ Nhập danh sách acc cho loại <b>{acc_type.upper()}</b>\n"
            "Mỗi dòng 1 acc, định dạng <code>tk:mk</code>\n"
            "Nhập <code>node</code> hoặc /cancel để hủy.",
            parse_mode="HTML"
        )
        return
        ...
        if approve_deposit(dep_id):
            ...
            try:
                await context.bot.send_message(
                    chat_id=int(uid),
                    text=f"🎉 <b>Đơn nạp đã được duyệt!</b>\nSố tiền: {amount:,} VND\nCảm ơn bạn.",
                    parse_mode="HTML"
                )
    ...
    if data.startswith("confirm_deposit_"):
        ...
        dep = get_deposit(dep_id)
        if not dep:
            await query.edit_message_text("❌ Lệnh nạp không tồn tại.")
            return
        if dep["status"] != "pending":
            await query.edit_message_text(f"⚠️ Lệnh này đã có trạng thái: {dep['status']}")
            return
...
async def process_purchase(query, user_id, acc_type, qty):
    ...
    if len(stock) < qty:
        await query.edit_message_text(f"❌ Không đủ acc! Còn {len(stock)} acc.", parse_mode="HTML")
        return
...
async def process_free_purchase(query, user_id, qty):
    ...
    if not can_get:
        await query.edit_message_text(f"❌ Hôm nay bạn đã lấy {used} acc (tối đa 3).", parse_mode="HTML")
        return
    remaining = 3 - used
    if qty > remaining:
        await query.edit_message_text(f"❌ Bạn chỉ có thể lấy thêm {remaining} acc hôm nay.", parse_mode="HTML")
        return
    total_coin = qty * 5
    if user["coins"] < total_coin:
        await query.edit_message_text(f"❌ Không đủ coin! Cần {total_coin}, có {user['coins']}", parse_mode="HTML")
        return
    stock = get_stock("free")
    if len(stock) < qty:
        await query.edit_message_text(f"❌ Không đủ acc free! Còn {len(stock)}", parse_mode="HTML")
        return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("awaiting_recharge"):
        try:
            ...
            caption = (
                ...
                f"📝 Nội dung: <code>{desc}</code>\n"
                f"💰 Số tiền: {amount:,} VND\n\n"
                "⚠️ <b>Phải ghi đúng nội dung chuyển khoản!</b>\n\n"
            ...
            await update.message.reply_photo(photo=qr_url, caption=caption, parse_mode="HTML", reply_markup=keyboard)
    ...
    if text == "Mua Acc Lv5":
        ...
        await update.message.reply_text(
            f"👤 <b>Tên:</b> {user.first_name}\n"
            f"🆔 <b>ID:</b> {user_id}\n"
            f"💰 <b>Số dư:</b> {user_data['balance']:,} VND\n"
            f"🪙 <b>Coin:</b> {user_data['coins']}\n"
            f"📊 <b>Tổng nạp:</b> {user_data['total_recharge']:,} VND",
            parse_mode="HTML"
        )
        return
