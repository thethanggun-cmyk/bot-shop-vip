...
app = Flask('')
@app.route('/')
def home(): return "CLONE_STORE_FULL_FIX_FINAL"
...
STK = "0356133246" # NHỚ SỬA LẠI SỐ CỦA MÀY Ở ĐÂY
...
class NapTienModal(Modal, title='THÔNG TIN CHUYỂN KHOẢN'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        async def confirm_callback(inter):
            ...
            await inter.response.send_message("Lệnh Nạp Đang Chờ Xử Lí💵", ephemeral=True)
...
class ShopMenu(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        # PHẢN HỒI NGAY LẬP TỨC ĐỂ FIX LỖI ĐỎ
        await interaction.response.defer(ephemeral=True)
