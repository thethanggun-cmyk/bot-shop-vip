...
# === API dành riêng cho admin: xem lịch sử nạp (cả pending và approved) trong 10 tiếng ===
...
def admin_approve_deposit():
    ...
    if db is not None:
        ...
        if target.get("timestamp", 0) < ten_hours_ago:
            return jsonify({"success": False, "message": "Lệnh đã quá hạn 10 tiếng, không thể duyệt!"}), 400
