...
def ensure_local_files():
    for f in [ACC_FILE, ACC_FREE_FILE, USERS_FILE, DEPOSITS_FILE, PURCHASES_FILE, INVITE_HISTORY_FILE]:
...
def confirm_invite():
    ...
    try:
        if db is not None:
            ...
            # Gửi tin nhắn cho người mời (retry 3 lần)
            msg_to_ref = f"🎉 **XÁC NHẬN MỜI THÀNH CÔNG!**\n\nBạn đã mời thành công @{new_username} (ID: `{user_id}`).\nBạn đã được cộng **+1 Coin**!"
            for attempt in range(3):
                try:
                    bot.send_message(ref_by, msg_to_ref, parse_mode="Markdown")
                    print(f"✅ Đã gửi tin nhắn cho người mời {ref_by}")
                    break
                except Exception as e:
                    print(f"❌ Lỗi gửi tin nhắn cho người mời {ref_by} (lần {attempt+1}/3): {e}")
                    time.sleep(2)
            else:
                print(f"❌ Không thể gửi tin nhắn cho người mời {ref_by} sau 3 lần thử.")

            # Gửi tin nhắn cho admin (retry 3 lần)
            ...
            for attempt in range(3):
                try:
                    ...
                    print(f"✅ Đã gửi tin nhắn cho admin {ADMIN_ID}")
                    break
                except Exception as e:
                    print(f"❌ Lỗi gửi tin nhắn cho admin {ADMIN_ID} (lần {attempt+1}/3): {e}")
                    time.sleep(2)
            else:
                print(f"❌ Không thể gửi tin nhắn cho admin {ADMIN_ID} sau 3 lần thử.")
...
def admin_get_deposits():
    if db is not None:
        items = list(db.deposits.find({"status": "pending"}, {"_id": 0}).sort("timestamp", -1).limit(50))
        return jsonify({"success": True, "deposits": items})
    else:
        with file_lock:
            ensure_local_files()
            with open(DEPOSITS_FILE, "r", encoding="utf-8") as f:
                deposits = json.load(f)
        pending = [d for d in deposits if d.get("status") == "pending"][:50]
