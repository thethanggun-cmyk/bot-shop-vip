...
app = Flask('')
@app.route('/')
def home(): return "CLONE_STORE_FIX_COMMANDS_V3"
...
# --- FIX LỖI LỆNH KHÔNG CHẠY ---
@bot.event
async def on_message(message):
    if message.author.bot: return
    # Dòng này cực kỳ quan trọng để lệnh ! hoạt động
...
class NapTienModal(Modal, title='THÔNG TIN CHUYỂN KHOẢN'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        async def confirm_callback(inter):
            ...
            await inter.response.send_message("Lệnh Nạp Đang Được Xử Lí Vui Lòng Đợi✅", ephemeral=True)
