...
def process_deposit_step(message):
    try:
        ...
        # Link hiển thị trực tiếp trên web, không bị bắt tải về
        qr_link = f"https://vietqr.net/image/{bank_id}-{stk}-compact2.jpg?amount={amount}&addInfo={noi_dung}"
        ...
        bot.send_message(message.chat.id, txt, reply_markup=mk, parse_mode="Markdown", disable_web_page_preview=True)
    except:
...
def handle_admin_callback(c):
    ...
    if c.data == "admin_stats":
        ...
        bot.send_message(ADMIN_ID, f"📦 **KHO**\n🛒 Lv5: {len(KHO_ACC['lv5'])}\n🛒 Kc: {len(KHO_ACC['kc'])}")
...
def add_acc_cmd(message):
    ...
    bot.reply_to(message, f"✅ Đã thêm acc {loai.upper()} vào kho!")
