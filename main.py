...
# --- [XỬ LÝ NẠP TIỀN - TÍCH HỢP SEPAY QR] ---
...
def process_deposit_step(message):
    try:
        ...
        # Link QR từ Sepay (Hiển thị web cực mượt, không lỗi)
        # Cấu hình: qr.sepay.vn/img?acc=<STK>&bank=<BANK>&amount=<TIEN>&des=<NOIDUNG>
        qr_link = f"https://qr.sepay.vn/img?acc={stk}&bank={bank_id}&amount={amount}&des={noi_dung}"
        ...
        # Gửi tin nhắn và hiện ảnh QR bên dưới (Preview)
...
def handle_admin_callback(c):
    ...
    if c.data == "admin_stats":
        if not USERS_DATA:
            bot.send_message(ADMIN_ID, "Chưa có dữ liệu.")
            return
...
def admin_action(c):
    ...
    if data[0] == "ok":
        ...
        bot.send_message(uid, "❌ Yêu cầu nạp đã bị từ chối.")
