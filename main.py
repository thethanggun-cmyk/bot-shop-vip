...
def get_user(user_id):
    """Lấy hoặc tạo user, trả về dict user."""
    uid = str(user_id)
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Xác nhận đã chuyển tiền (từ user)
    ...
    if data.startswith("qty_7day_") or data.startswith("qty_24h_"):
        parts = data.split("_")
        acc_type = parts[1]  # 7day hoặc 24h
        qty_str = parts[2]
        if qty_str == "custom":
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Nhập số lượng tùy chỉnh (mua acc Lv5)
    if context.user_data.get("awaiting_custom_qty"):
        ...
        # Tạo một fake query để gọi process_purchase
        class FakeQuery:
    ...
    if context.user_data.get("awaiting_recharge"):
        try:
            ...
            if amount <= 0:
                raise ValueError
            # Tạo yêu cầu nạp (trạng thái pending)
...
def main():
    ...
    # Khởi tạo DB nếu cần (không cần, vì đã có index)
