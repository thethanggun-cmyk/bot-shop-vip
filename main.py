...
@app.route('/')
def home():
    return "Bot is Running!" # Link này dùng để dán vào UptimeRobot
...
def admin(message):
    ...
    bot.send_message(ADMIN_ID, f"📊 **ADMIN PANEL**\n👤 Khách: {u_count}\nLệnh: `/users`, `/add lv5 acc1,acc2`, `/setmoney ID tiền`")
...
def set_money(message):
    ...
    try:
        ...
        bot.send_message(ADMIN_ID, f"✅ Đã cộng {money}đ cho {uid}")
    except: pass
...
def handle_msg(m):
    if m.text == "🛒 Mua Acc Lv5/Kc":
        ...
        bot.register_next_step_handler(msg, lambda ms: bot.send_message(ms.chat.id, f"🏦 MB BANK\nSTK: `12345678` (Chủ TK)\nND: `NAP {ms.from_user.id}`", 
