...
app = Flask('')
@app.route('/')
def home(): return "CLONE_GUN_STORE_FINAL_V6"
...
class NapTienModal(Modal, title='NẠP TIỀN QUA PAYOS'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="🔗 Link:", value=f"[Thanh toán tại đây](https://me.payos.vn/v2/auth/7710092008)", inline=False)
        ...
        async def confirm_callback(inter):
            try:
                ...
                embed_ad = discord.Embed(title="🔔 YÊU CẦU NẠP TIỀN", color=discord.Color.yellow())
...
class BuyQuantityModal(Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.followup.send(f"❌ Mở DM nhận acc nhé:\n{acc_text}", ephemeral=True)
...
async def nhap_lv5(ctx, *, accs: str):
    ...
    save_data(data)
    await ctx.send(f"✅ Đã thêm {len(list_acc)} acc LV5.")
