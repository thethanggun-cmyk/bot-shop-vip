...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("confirm_deposit_"):
        ...
        # Thông báo cho user
        await query.edit_message_text(
            "✅ Bạn đã tạo đơn nạp tiền thành công!\n"
            "Vui lòng chờ admin duyệt (thường vài phút).\n"
...
async def process_purchase(query, user_id, acc_type, qty):
    ...
    if len(stock) < qty:
        await query.edit_message_text(f"❌ Không đủ acc! Còn {len(stock)} acc.")
        return
...
async def process_free_purchase(query, user_id, qty):
    ...
    if not can_get:
        await query.edit_message_text(f"❌ Hôm nay bạn đã lấy {used} acc (tối đa 3).")
        return
    remaining = 3 - used
    if qty > remaining:
        await query.edit_message_text(f"❌ Bạn chỉ có thể lấy thêm {remaining} acc hôm nay.")
        return
    total_coin = qty * 5
    if user["coins"] < total_coin:
        await query.edit_message_text(f"❌ Không đủ coin! Cần {total_coin}, có {user['coins']}")
        return
    stock = get_stock("free")
    if len(stock) < qty:
        await query.edit_message_text(f"❌ Không đủ acc free! Còn {len(stock)}")
        return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("awaiting_recharge"):
        try:
            ...
            await update.message.reply_photo(photo=qr_url, caption=caption, reply_markup=keyboard)
