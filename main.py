...
def send_welcome(message):
    ...
    # Lưu người dùng đã /start vào DB/File
    if db is not None:
        db.bot_users.update_one({"userId": user_id}, {"$set": {"username": username}}, upsert=True)
    else:
        ensure_local_files()
    ...
    web_app_url = "https://t.me/thethanggun_bot/webapp"
    if ref_id:
...
def broadcast_message(message):
    ...
    users_list = []
    if db is not None:
        users_list = [u["userId"] for u in db.bot_users.find({}, {"userId": 1})]
    else:
        ensure_local_files()
        with open(USERS_FILE, "r", encoding="utf-8") as f: users = json.load(f)
...
def ban_user(message):
    ...
    if db is not None:
        ...
        if user:
            db.users.update_one({"userId": user["userId"]}, {"$set": {"is_banned": True}})
            bot.reply_to(message, f"🚫 Đã cấm ID/User `{user['userId']}` đổi Acc Free Lv5!")
        else:
            db.users.update_one({"userId": target}, {"$set": {"is_banned": True}}, upsert=True)
...
# ==================== API USER & CỘNG COIN THÔNG BÁO MỜI THÀNH CÔNG ====================
...
def get_user_info():
    ...
    if db is not None:
        ...
        if not user:
            ...
            # Xử lý thông báo khi mời thành công
            if ref_by and ref_by != user_id:
                db.users.update_one({"userId": ref_by}, {"$inc": {"coins": 1}}, upsert=True)
                
                # Gửi thông báo cho Người mời & Admin
                try:
                    ...
                    bot.send_message(ADMIN_ID, f"🔔 **THÔNG BÁO GIỚI THIỆU**\nUser `{ref_by}` đã mời thành công người dùng mới `{user_id}` vào Mini App!", parse_mode="Markdown")
                except Exception:
                    pass
...
# API ĐỔI ACC BẰNG COIN (KIỂM TRA BANNED + MAX 3 ACC / NGÀY)
