    ...
    if db is not None:
        ...
        if not user:
            db.users.insert_one({
                ...
                "is_banned": False,
                "confirmed_invite": False  # chưa xác nhận mời
            })
...
def confirm_invite():
    ...
    # Kiểm tra xem user này đã xác nhận mời chưa
    if db is not None:
        user = db.users.find_one({"userId": user_id})
        if not user:
        ...
        new_tag = f"@{new_user.get('username')}" if new_user.get('username') else f"`{user_id}`"
        
        try:
            bot.send_message(ref_by, "🎉 **XÁC NHẬN MỜI THÀNH CÔNG!**\n\nBạn đã giới thiệu thành công 1 người dùng mới và nhận được **+1 Coin**!", parse_mode="Markdown")
            bot.send_message(ADMIN_ID, f"🔔 **THÔNG BÁO MỜI THÀNH CÔNG**\nUser {ref_tag} đã mời thành công người dùng {new_tag} vào Mini App (đã xác nhận qua câu đố)!", parse_mode="Markdown")
        except Exception as e:
        ...
        # Gửi thông báo
        ref_tag = f"@{users[ref_by].get('username')}" if users[ref_by].get('username') else f"`{ref_by}`"
        new_tag = f"@{users[user_id].get('username')}" if users[user_id].get('username') else f"`{user_id}`"
        try:
            bot.send_message(ref_by, "🎉 **XÁC NHẬN MỜI THÀNH CÔNG!**\n\nBạn đã giới thiệu thành công 1 người dùng mới và nhận được **+1 Coin**!", parse_mode="Markdown")
            bot.send_message(ADMIN_ID, f"🔔 **THÔNG BÁO MỜI THÀNH CÔNG**\nUser {ref_tag} đã mời thành công người dùng {new_tag} vào Mini App (đã xác nhận qua câu đố)!", parse_mode="Markdown")
        except Exception as e:
