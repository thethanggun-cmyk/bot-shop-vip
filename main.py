...
def process_deposit_step(message):
    try:
        ...
        uid = message.from_user.id
        stk = "04312345" 
        bank_id = "OceanBank" # Mã chuẩn để không bị lỗi 404
        noi_dung = f"NAP{uid}"
        
        # Dùng .jpg để iPhone hiện thẳng ảnh, không bắt tải về
        qr_link = f"https://img.vietqr.io/image/{bank_id}-{stk}-compact2.jpg?amount={amount}&addInfo={noi_dung}"
        
        txt = (
            f"🏦 **MBV BANK (OceanBank)**\n"
            f"💳 **STK:** `{stk}`\n"
            f"💰 **Tiền:** `{amount:,}đ`\n"
            f"📝 **ND:** `{noi_dung}`\n"
        ...
        # disable_web_page_preview=False giúp ảnh hiện ngay trong tin nhắn Telegram
        bot.send_message(message.chat.id, txt, reply_markup=mk, parse_mode="Markdown", disable_web_page_preview=False)
    except:
        bot.send_message(message.chat.id, "❌ Lỗi! Vui lòng nhập số tiền hợp lệ.")
...
def admin_action(c):
    ...
    if data[0] == "ok":
        ...
        bot.send_message(uid, "❌ Yêu cầu nạp của bạn bị từ chối.")
...
def finish_buy(m, loai):
    try:
        ...
        if USERS_DATA[uid]["money"] < qty * gia:
            bot.send_message(m.chat.id, "❌ Ví không đủ tiền!"); return
        if len(KHO_ACC[loai]) < qty:
            bot.send_message(m.chat.id, "❌ Kho hàng không đủ!"); return
        accs = [KHO_ACC[loai].pop(0) for _ in range(qty)]
        USERS_DATA[uid]["money"] -= qty * gia
        bot.send_message(m.chat.id, f"✅ Thành công!\n🔑 Danh sách Acc:\n" + "\n".join(accs))
    except: bot.send_message(m.chat.id, "❌ Lỗi! Vui lòng nhập số.")
