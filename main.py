...
db = None
file_lock = threading.Lock()  # Dùng cho fallback file
...
if MONGO_URI:
    try:
        ...
        db = client["gun_store"]
        # Tạo index cho các collection thường dùng
...
def list_users(message):
    ...
    if db is not None:
        # Lấy tối đa 100 user để tránh quá tải
    ...
    msg = f"📊 **DANH SÁCH NGƯỜI DÙNG (hiển thị {len(users_list)})**:\n\n"
...
def get_user_info():
    ...
    if db is not None:
        # Dùng find_one_and_update để tránh 2 query
...
# === Các API còn lại giữ nguyên (get-stock, acclv5, buy-acc, redeem-free-acc, get-purchases, request-deposit, get-deposit-history, admin/get-deposits, admin/approve-deposit, is-admin) ===
# Tôi sẽ giữ nguyên các hàm đó, chỉ thêm lock cho file I/O.
