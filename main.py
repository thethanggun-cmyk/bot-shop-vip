...
# --- [QUY TRÌNH NẠP TIỀN - OCEANBANK (MBV)] ---
...
def process_deposit_step(message):
    try:
        ...
        uid = message.from_user.id
        stk = "04312345" # <-- Thay số tài khoản OceanBank của bạn vào đây
        bank_id = "oceanbank" # Mã chuẩn của Ngân hàng Đại Dương
        noi_dung = f"NAP{uid}"
        
        # Link QR cho OceanBank với tên hiển thị MBV BANK
        qr_link = f"https://img.vietqr.io/image/{bank_id}-{stk}-compact2.png?amount={amount}&addInfo={noi_dung}&accountName=MBV%20BANK"
