...
def send_welcome(message):
    ...
    # Gửi URL kèm tham số giới thiệu
    web_app_url = f"https://t.me/thethanggun_bot/webapp?startapp={ref_id}" if ref_id else "https://t.me/thethanggun_bot/webapp"
...
def get_user_info():
    ...
    if db is not None:
        ...
        if not user:
            ...
            # Xử lý khi có người giới thiệu hợp lệ
            if ref_by and ref_by != user_id:
                # Cộng 1 coin cho người mời
                db.users.update_one({"userId": ref_by}, {"$inc": {"coins": 1}}, upsert=True)
                
                # Tìm tên / ID người mời & người được mời để gửi thông báo
            ...
            # Cập nhật lại username mới nhất nếu có
...
# API ĐỔI ACC BẰNG COIN (5 COIN = 1 ACC FREE LV5, MAX 3 ACC / NGÀY)
