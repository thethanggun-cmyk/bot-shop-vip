...
def ensure_local_files():
    ...
    if not os.path.exists(INVITE_HISTORY_FILE):
        with open(INVITE_HISTORY_FILE, "w", encoding="utf-8") as f: json.dump([], f)
...
def send_welcome(message):
    ...
    # Tạo 2 button: Mở Mini App và Admin
...
# Xử lý callback khi nhấn nút Admin
@bot.callback_query_handler(func=lambda call: call.data == "admin_info")
def admin_callback(call):
    bot.answer_callback_query(call.id)  # Xóa hiệu ứng loading
...
def confirm_invite():
    ...
    if db is not None:
        ...
        db.invite_history.insert_one(invite_record)
...
def admin_get_invite_history():
    ...
    if db is not None:
        records = list(db.invite_history.find({}, {"_id": 0}).sort("raw_time", -1))
