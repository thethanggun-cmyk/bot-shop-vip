...
# ==================== API USER & HỆ THỐNG COIN GIỚI THIỆU ====================
@app.route("/get-user-info", methods=["GET"])
def get_user_info():
    user_id = str(request.args.get("userId", ""))
    ref_by = str(request.args.get("refBy", ""))  # ID người giới thiệu nếu có
    ...
    if db is not None:
        ...
        if not user:
            # Tạo tài khoản mới
            db.users.insert_one({"userId": user_id, "balance": 0, "coins": 0, "referred_by": ref_by if ref_by != user_id else ""})
            
            # Nếu được giới thiệu bởi người khác -> Cộng +1 Coin cho người giới thiệu
        ...
        if user_id not in users:
            users[user_id] = {"balance": 0, "coins": 0, "referred_by": ref_by if ref_by != user_id else ""}
...
# API ĐỔI ACC BẰNG COIN (4 COIN = 1 ACC FREE LV5)
...
def redeem_free_acc():
    ...
    if db is not None:
        user = db.users.find_one({"userId": user_id}) or {"coins": 0}
        ...
        ensure_local_files()
        with open(USERS_FILE, "r", encoding="utf-8") as f: users = json.load(f)
        user_coins = users.get(user_id, {}).get("coins", 0)
        ...
        with open(PURCHASES_FILE, "r", encoding="utf-8") as f: purchases = json.load(f)
        time_str = time.strftime("%H:%M - %d/%m/%Y")
        purchases.insert(0, {"userId": user_id, "accs": "\n".join(bought_accs), "count": len(bought_accs), "time": time_str + " (Đổi bằng Coin)"})
        with open(PURCHASES_FILE, "w", encoding="utf-8") as f: json.dump(purchases, f, indent=2)
...
# ==================== API NẠP TIỀN & ĐIỀU KIỆN 20 PHÚT ====================
