...
app = Flask('')
@app.route('/')
def home(): return "CLONE_STORE_ANTI_RESET_V4"
...
# --- HÀM LƯU TRỮ CHỐNG MẤT DỮ LIỆU ---
def load_data():
    # Mày có thể điền sẵn acc vào đây để dự phòng nếu file bị xóa
    backup_stock = {"kc": [], "lv5": []} 
    ...
    if os.path.exists(DATA_FILE):
        try:
            with open(DATA_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
                # Đảm bảo cấu trúc file luôn đúng
                if "users_balance" not in data: data["users_balance"] = {}
                if "stock" not in data: data["stock"] = backup_stock
                return data
        except: pass
    return {"users_balance": {}, "stock": backup_stock}
...
async def on_message(message):
    ...
    if content.startswith("!nhap_"):
        message.content = content.replace("!nhap_", "!nhap ")
    await bot.process_commands(message)

# --- VIEW DUYỆT TRONG DM ADMIN ---
class AdminApproveView(discord.ui.View):
    ...
    async def approve(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            await user.send(f"✅ **THÀNH CÔNG:** Admin đã duyệt đơn `{self.order_id}`. Bạn đã được cộng **{self.amount:,} VNĐ**!")
        except: pass
...
class ShopMenu(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if balance < gia:
            return await interaction.followup.send(f"❌ Thiếu {gia - balance:,}đ.", ephemeral=True)
        ...
        try:
            ...
            await interaction.followup.send(f"❌ Mở DM nhận hàng: `{acc}`", ephemeral=True)
...
async def setup_shop(ctx):
    data = load_data()
    embed = discord.Embed(title="🛒 HỆ THỐNG BÁN ACC TỰ ĐỘNG", color=0x2b2d31)
...
async def nhap(ctx, loai: str, *, accs: str):
    ...
    data = load_data()
    list_acc = [a.strip() for a in accs.replace('\n', ',').split(",") if a.strip()]
